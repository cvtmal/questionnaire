        ...
        <TooltipProvider>
            <Tooltip>
                <TooltipTrigger asChild>
                    <div
                        className={`flex max-w-full items-center space-x-1 ${onClick ? 'hover:text-primary cursor-pointer transition-colors' : ''}`}
                        onClick={onClick}
                    >
...
const SortHeader = ({
    ...
    return (
        <div className="flex cursor-pointer items-center space-x-1" onClick={() => onSort(column)}>
            <span>{children}</span>
                    ...
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className="w-[250px]">
                                <div className="flex items-center">Job</div>
                            </TableHead>
                            <TableHead className="w-[80px]">
                                <div className="flex items-center">Workload</div>
                            </TableHead>
                        ...
                        {questionnaires.data.length === 0 ? (
                            ...
                            questionnaires.data.map((questionnaire) => (
                                <TableRow key={questionnaire.id}>
                                    ...
                                    <TableCell>{questionnaire.created_at ? format(new Date(questionnaire.created_at), 'PP') : '-'}</TableCell>
                                    <TableCell>{questionnaire.updated_at ? format(new Date(questionnaire.updated_at), 'PP') : '-'}</TableCell>
                                    <TableCell className="text-right">
                                        <div className="flex justify-end space-x-1">
                                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleOpenDetails(questionnaire)}>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 sm:flex-row">
                ...
                {questionnaires.meta && (
                    <div className="text-muted-foreground text-sm">
                        Showing {questionnaires.meta.from || 0} to {questionnaires.meta.to || 0} of {questionnaires.meta.total || 0} entries
                    </div>
                )}
                    ...
                    {questionnaires.meta && questionnaires.meta.last_page > 1 && (
                        <Pagination className="my-2">
                            <PaginationContent>
                                ...
                                {questionnaires.meta.links &&
                                    questionnaires.meta.links.slice(1, -1).map((link, i) => {
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="sm:max-w-[625px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>Full information about this questionnaire submission.</DialogDescription>
                    </DialogHeader>
                    {selectedQuestionnaire && <QuestionnaireDetailView questionnaire={selectedQuestionnaire} />}
                </DialogContent>
            </Dialog>
        </div>
    );
